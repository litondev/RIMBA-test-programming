	    ...
	    <div v-if="!loadingContent"
	    	...
	    	<div class="clearfix mb-3">
	    		...
	    		<div class="float-right">
		    		<router-link to="/item/form/0" class="btn btn-primary">
	    				Tambah
		    		</router-link>
		    	</div>
		    </div>
			...
			<table class="table table-hover mt-2">
				...
				<tr v-for="(item,index) in theData">
					<td>{{item.id}}</td>
					<td>
						<img :src="item.barang" style="width:200px;height:200px;object-fit:cover">
					</td>
					<td>{{item.nama_item}}</td>
					<td>{{item.unit}}</td>
					<td>{{item.stock}}</td>
					<td>
						<router-link :to="'/item/form/'+item.id" class="btn btn-success">Edit</router-link>
